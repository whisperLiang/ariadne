...
from ariadne.benchmark import BenchmarkResult, benchmark_replay_optimization
...
def _run_model(
    ...
    *,
    ...
    for batch_size in batches:
        ...
        results = benchmark_replay_optimization(
            model,
...
def _print_rows(rows: list[tuple[str, int, list[BenchmarkResult]]]) -> None:
    print(
        "| model | batch | metric | avg_ms | speedup_vs_baseline | backend | fallback |"
    )
    print("|---|---:|---|---:|---:|---|---|")
    for model_name, batch_size, results in rows:
        by_name = {result.name: result for result in results}
        baseline = by_name["baseline_generated_prefix_suffix"].average_latency_s
        for result in results:
            ...
            print(
                ...
                f"{result.fallback_reason or ''} |"
            )


def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark optimized split replay inference.")
    parser.add_argument(
        "--models",
        nargs="+",
        default=["all"],
    ...
    parser.add_argument(
        "--no-materialize-boundary",
        action="store_true",
        help="Skip cloning compiled prefix outputs before storing them in ReplayBoundary.",
    )
    parser.add_argument(
        "--device",
    ...
    parser.add_argument(
        "--require-cuda",
        action="store_true",
        help="Fail fast instead of falling back to CPU when CUDA is unavailable.",
    )
    ...
    for model_name in selected:
        all_rows.extend(
            _run_model(
                ...
                materialize_boundary=not args.no_materialize_boundary,
                device=device,
            )
        )
